...
function makeName(s: string, uniq = false): Name {
	if (uniq) {
		return {
			type: "name",
			symb: `_${s}_${Math.random().toString(36).substring(7)}`,
		};
	}
...
function isTie(t: any): t is Tie {
	return (
		typeof t === "object" && "type" in t && t.type === "tie"
	);
}
...
function availableo(n1: Term, t1: Term): Goal {
	return (state): Streamof<State> => {
		...
		if (isName(n)) {
			...
			if (!r)
				return [{ sigma, chi, fenv, fenv2, inc }, null];
...
function fresh3(
	f: (n1: Name, n2: Name, n3: Name) => Goal,
): Goal {
...
function exist3(
	f: (v1: LVar, v2: LVar, v3: LVar) => Goal,
): Goal {
	return exist((v1) => exist2((v2, v3) => f(v1, v2, v3)));
}
function exist4(
	f: (v1: LVar, v2: LVar, v3: LVar, v4: LVar) => Goal,
): Goal {
	return exist((v1) =>
		exist3((v2, v3, v4) => f(v1, v2, v3, v4)),
	);
}
...
function projectRun(
	...
	return runGoal(i, g).map((s) =>
		vs.map((v) => walkStar(s.sigma, v)),
	);
}
...
function unif(
	...
	if (isName(t1) && isName(t2)) {
		return sameName(phi1, t1, phi2, t2)
			? [sigma, chi, xs]
			: false;
	}
	if (isName(t1) && isLVar(t2)) {
		...
		return n2
			? [extSubst(sigma, t2, n2), chi, [t2, ...xs]]
			: false;
	}
	if (isLVar(t1) && isName(t2)) {
		...
		return n1
			? [extSubst(sigma, t1, n1), chi, [t1, ...xs]]
			: false;
	}
	if (isTie(t1) && isTie(t2)) {
		return unif(
...
function validateFenv(
	...
	if (ls) {
		const newFenv = validateFenvEach(
			sigma.get(x.symb),
			ls,
			fenv,
		);
...
function equals(t1: Term, t2: Term): Goal {
	...
	return (state): Streamof<State> => {
		...
		if (result) {
			...
			if (newFenv) {
				const fenv2Validation = validateFenv2(
					xs,
					newSigma,
					newFenv,
					fenv2,
				...
				if (fenv2Validation) {
					const [validatedFenv, validatedFenv2] =
						fenv2Validation;
					return [
						{
							sigma: newSigma,
		...
		console.warn(
			"equals: Unification failed1",
			reifyString(t1, state),
			reifyString(t2, state),
		);
		return null;
	};
}
...
const lstringify = (t: Term): string => {
	...
	if (isTie(t))
		return `tie(${lstringify(t.name)}, ${lstringify(t.term)})`;
	if (Array.isArray(t))
		return `[${lstringify(t[0])}, ${lstringify(t[1])}]`;
...
function subst(
	...
	return disj(
		conj(equals(qvar(a), e), equals(nnew, out)),
		exist((y) => {
			return conj(
				equals(qvar(y), e),
				equals(qvar(y), out),
