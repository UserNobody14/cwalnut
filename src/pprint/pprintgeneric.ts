...
import { indentStr } from "../pprint/indentStr";
import type {
	TermGeneric,
	ExpressionGeneric,
	TermT,
...
export function pprintGeneric<T>(
	...
	if (Array.isArray(ast)) {
		return ast
			.map((a) => pprintGeneric<T>(a, printMeta))
			.join("\n");
	}
	switch (ast.type) {
		case "conjunction":
...
export function pprintType(ast: Type): string {
	switch (ast.type) {
		...
		case "complex":
			return `${ast.name}${
				ast.fresh.length > 0
					? `[${ast.fresh.map(pprintType).join(", ")}]`
					: ""
			}<${ast.generics.map(pprintType).join(", ")}>`;
	}
}
export function pprintTypeMeta(
	ctx: CtxType,
	meta: Type,
): string {
	return ctx.withtype === "withtype"
		? `: ${pprintType(meta)}`
		: "";
}
export function pprintTermT(
	terms: TermT | TermT[],
): string {
	return pprintTermTFlex(terms, "withtype");
}
export function pprintTermTFlex(
	terms: TermT | TermT[],
	displayTypes: "withtype" | "withouttype",
): string {
	return pprintGeneric(
		terms,
