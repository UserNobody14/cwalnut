...
export function usingMappedCodeExprs<T>(
  ...
  const mappedExprs: FullExpression<CodeLocation>[] = exprs.map((expr) =>
    deflex(expr),
  );
    ...
    get:
      ...
      (...args: CodeExpr[]) =>
        usingMappedCodeExprs(args, (mappedArgs, terms) =>
          conj1(...terms, call(prop.toString(), ...mappedArgs)),
        ),
  },
);
...
export const cl2: Record<string, (...args: CodeExpr[]) => CodeExpr> = new Proxy(
  {},
  {
    get:
      (_, prop) =>
      (...args: CodeExpr[]) =>
        make_pred_expr(prop.toString(), args, defaultCodeLocation, null, 0),
  },
);

export function lit(value: string | number | boolean | null) {
  if (value === null) {
...
export function conj1(...terms: TermGeneric<CodeLocation>[]) {
  if (terms.length === 0) {
...
export function call(
  source: string,
  ...args: (ReturnType<typeof id> | ReturnType<typeof lit>)[]
) {
