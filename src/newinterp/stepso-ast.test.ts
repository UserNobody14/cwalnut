...
function cxlam(output: ReturnType<typeof id>, input: ReturnType<typeof id>) {
...
(substo (tie a E1) E E1^))))
     */
        freshNominal1(
          [cx.A, cx.B],
          fresh1(
            [cx.E1, cx.E1Hat, cx.Body, cx.LamBody, cx.TieAE1, cx.OutTie],
            cl.hash(cx.B, cx.E),
  ...
  return def(
    ...
    conj(
      disjunction1(
        ...
        fresh1([cx.T], cl.stepo(cx.T1, cx.T), cl.stepso(cx.T, cx.T2)),
      ),
    ),
  );
}
...
function stepsoAst(): TermGeneric<CodeLocation>[] {
  return [substo2Def(), betaoDef(), stepoDef(), stepsoDef()];
}
...
describe("stepso via AST (stepso2.test.ts)", () => {
  test("Logic 45: reflexive transitive closure", () => {
    ...
    expect(states).toEqual([
      {
        q: qapp2(
          qlam2(tieTag("Nom(0)", qlam2(tieTag("Nom(1)", qvar2("Nom(0)"))))),
          qvar2("Nom(1)"),
        ),
      },
    ]);
  ...
  test("Logic 47: stepso M N, first 3 answers", () => {
    const main = fresh1(
      [cx.M, cx.N],
      conj(cl.stepso(cx.M, cx.N), cl.list(cx.q, cx.M, cx.N)),
    );
    const ast = [...stepsoAst(), main];
    ...
    expect(states).toEqual([
      { q: ["?$&0", "?$&0"] },
      {
        q: [qapp2(qlam2(tieTag("Nom(0)", qvar2("Nom(0)"))), "?$&0"), "?$&0"],
      },
      {
        q: [
          qapp2(
...
describe("stepso via AST (stepso3.test.ts)", () => {
  test("Logic 56", () => {
    ...
    const body2 = qapp2(qvar2("Nom(0)"), qappVV2("Nom(2)", "Nom(2)"));
    expect(states).toEqual([
      {
        t: qapp2(
          qvar2("Nom(0)"),
          qapp2(qlam2(tieTag("Nom(1)", body1)), qlam2(tieTag("Nom(2)", body2))),
        ),
      },
    ]);
  }, 60000);
