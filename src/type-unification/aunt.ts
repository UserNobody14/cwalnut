...
import { pprintType } from "src/pprint/pprintgeneric";
...
const is_predicate = (type: Type): type is ComplexType => {
	return (
		type.type === "complex" && type.name === "predicate"
	);
};
...
function unifyUnionBoxed(
	union: UnionType,
	t: Type,
	subst: ImmMap<string, Type>,
): string | ImmMap<string, Type> {
	if (union.types.length === 0) {
		throw new Error(
			`Cannot unify empty union type with type ${pprintType(t)}`,
		);
	}
...
function unifyUnion(
	...
	throw new Error(
		`Cannot unify union type ${pprintType(union)} with type ${pprintType(t)}`,
	);
	/**
     * in env:
    ${pprintTypeMap(subst)}
...
function unifyParametric(
	...
	if (
		...
		throw new Error(
			`Cannot unify parametric types: ${pprintType(t1)} and ${pprintType(t2)}`,
		);
	}
...
function unifyVar(
	...
	if (occursCheck(v, t, subst)) {
		throw new Error(
			`Occurs check failed: ${pprintType(v)} in ${pprintType(t)}`,
		);
	}
...
export function occursCheck(
	...
	if (t.type === "variable") {
		return (
			v.name === t.name ||
			(subst.has(t.name) &&
				occursCheck(v, subst.get(t.name) as Type, subst))
		);
	}
	if (t.type === "union") {
...
function instantiate(type: Type): Type {
	if (type.type === "complex" && type.fresh.length > 0) {
		...
		for (const varName of type.fresh) {
			subst = subst.set(
				varName.name,
				make.type_variable(generateFreshVarName()),
			);
		}
		const instantiatedGenerics = type.generics.map((g) =>
			applySubstitution(g, subst),
		);
...
function applySubstitution(
	...
	if (type.type === "union") {
		...
		return make.union_type(
			...type.types.map((t) => applySubstitution(t, subst)),
		);
	}
...
export function unifyTwoMaps(
	subst1: ImmMap<string, Type>,
	subst2: ImmMap<string, Type>,
): ImmMap<string, Type> {
	let newSubst = subst1;
	for (const [k, v] of subst2.entries()) {
		const bk = subst1.get(k);
		newSubst = newSubst.set(k, softUnify(v, bk, newSubst));
	}
	for (const k of ImmSet(subst1.keys()).subtract(
		...
		if (!v)
			throw new Error(
				`Unification failed: ${k} not found in subst2`,
			);
