...
const tmap = (obj?: Record<string, Type>): Tmap =>
	obj ? ImmMap(obj) : ImmMap<string, Type>();

const tstring = make.simple_type("string");
const tnumber = make.simple_type("number");
const tpred = (
	frsh: (string | TypeVariable)[],
	tt: Type[],
) =>
	make.complex_type(
		"predicate",
		frsh.map((ff) =>
			typeof ff === "string" ? make.type_variable(ff) : ff,
		),
		tt,
	);
const tlist = (
...
describe("Type unification", () => {
	...
	test(" instantiation of a predicate 5", () => {
		checkUnity(
			tvar("predtest"),
			tpred([], [tnumber, tvar("x2")]),
			tmap({
				// predtest: tpred( [], [tnumber, tnumber])
				// predtest: tpred( [tvar('P1')], [tvar('P1'),tvar('P1')]),
				predtest: tpred(
					[tvar("P1")],
					[tvar("P1"), tlist([], [tvar("P1")])],
				),
			}),
			tmap({
				predtest: tpred(
