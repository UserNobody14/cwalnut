	...
	switch (a.type) {
		...
		case "union":
			return (
				...
				a.types.every((t, i) => compareTypes(t, b.types[i]))
			);
		case "complex":
			return (
				b?.type === "complex" &&
				...
				a.fresh.every((t, i) =>
					compareTypes(t, b.fresh[i]),
				) &&
				a.generics.length === b.generics.length &&
				a.generics.every((t, i) =>
					compareTypes(t, b.generics[i]),
				)
			);
	}
}
export function compareTypeMaps(
	a1: ImmMap<string, Type>,
	b1: ImmMap<string, Type>,
): boolean {
	const a = reify(a1);
	const b = reify(b1);
