...
export function reifyType(
	...
	if (type.type === "variable") {
		...
		if (occursCheck(type, t, subst)) {
			throw new Error(
				`Occurs check failed: ${pprintType(type)} in ${pprintType(t)}`,
			);
		}
	...
	if (type.type === "union") {
		...
		// }
		const types2 = collapseUnion(
			type.types.map((t) => reifyType(t, subst)),
		);
		if (types2.length === 1) {
			return types2[0];
		}
	...
	if (type.type === "complex") {
		const finGen = type.generics.map((t) =>
			reifyType(t, subst),
		);
		return make.complex_type(
			type.name,
			type.fresh.filter((v) =>
				finGen.some((g) => occursCheck(v, g, subst)),
			),
			finGen,
		);
	}
