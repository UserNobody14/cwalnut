...
function toSouffleObj(
	...
	switch (term.type) {
		case "predicate_definition": {
			...
			allLists.code_location.push({
				id: statement_id,
				...name.info,
			});
			for (let idx = 0; idx < args.length; idx++) {
				const prm = args[idx].value;
				allLists.parameter_declaration.push({
					param_id,
					idx,
					prm,
				});
			}
			allLists.pred_def.push({
				id: pred_id,
				name: name_string,
				param_id,
				statement_id,
		...
		case "disjunction": {
			...
			for (let idx = 0; idx < terms.length; idx++) {
				...
				allLists.disjunction_statement.push({
					id,
					idx,
					further_st,
				});
				allLists = toSouffleObj(
					terms[idx],
					further_st,
					allLists,
				);
		...
		case "conjunction": {
			...
			for (let idx = 0; idx < terms.length; idx++) {
				const further_st = getNewId();
				allLists.conjunction_statement.push({
					id,
					idx,
					further_st,
				});
		...
		case "predicate_call": {
			...
			allLists.predicate_call_statement.push({
				stid,
				name,
				idx,
				params,
			...
			for (let idx = 0; idx < args.length; idx++) {
				const prm = args[idx].value;
				const isliteral =
					args[idx].type === "literal" ? 1 : 0;
				if (args[idx].type === "literal") {
					const literal_id = getNewId();
					// TODO allow nums & bools too.
					allLists.literal_strings.push({
						literal_id,
						str: prm,
					});
					allLists.predicate_call_parameter.push({
						param_id: params,
						idx,
						prm: literal_id,
						isliteral,
...
function toSouffleCsvStringOrdered(
	valueList: AllLists[keyof AllLists],
) {
	return valueList
		.map((row) => {
