...
export function interpretPlus(ast: TermTyped | TermTyped[]): WorldGoal {
    ...
    switch (ast.type) {
        ...
        case 'disjunction':
            return WorldGoal.disj(...ast.terms.map(interpretPlus));
        case 'predicate':
            const ast2333 = interpretExprListPlus(ast.args, 'without_type', 'without_io');
            // console.log('predicate', ast.source.value, ast2333.map(a => a.toString()));
            return WorldGoal.pred(
                ast.source.value,
                ast2333
            );
                ...
                new LogicPredicate(
                    new PredicateLogic((...args: LogicValue[]) => {
                        return new WorldGoal((world) => {
                            const newWorld1 = new World(builtinWorld.lvars);
                            for (let i = 0; i < ast.args.length; i++) {
                                // const freshened = world.walk(args[i]);
                                newWorld1.addLvar(ast.args[i].value, args[i]);
                            }
                            ...
                            return new WorldGoal((world2) => {
                                ...
                                const res2 = new World(new Map(world.lvars));
                                // const res2 = world
                                ...
                                //     }
                                // }
                                const walkedArgsReturned = ast.args.map((arg) => {
                                    const respLvar = world2.getLvar(arg.value);
                                    if (respLvar) {
                                ...
                                if (walkedArgsReturned.some((w) => w === undefined)) {
                                    return [World.fail];
                                }
                                const walkGoals = walkedArgsReturned.map((walked, i) => eq(args[i], walked));
                                return WorldGoal.and(...walkGoals).run(res2);
                            }).runAll(res);
...
function interpretExprListPlus(ast: ExpressionTyped[], withtype: 'withtype' | 'without_type' = 'without_type',
    withio: 'withio' | 'without_io' = 'without_io'): LogicValue[] {
    return ast.map(ee => interpretExpr(ee, withtype, withio));
}
function interpretExpr(ast: ExpressionTyped, withtype: 'withtype' | 'without_type' = 'without_type',
    withio: 'withio' | 'without_io' = 'without_io'): LogicValue {
    switch (ast.type) {
        case 'identifier':
...
function runWorlds(goal: WorldGoal, worlds: World[] = [
    new World(new Map())
]): World[] {
...
export const interpretFromCode = (code: string, lvars1: string[]) => {
