...
describe("Test1", () => {
	...
	test("Kn First", () => {
		const firsto = (a: kn.LTerm, l: kn.LTerm): kn.Goal => {
			return kn.call_fresh((v) =>
				kn.eq(kn.makePair(a, v), l),
			);
		};
		const outv = kn.run(
			3,
    ...
    qq = [45]
		...
		const outv = kn.run(
			...
			kn.all(
				...
				kn.eq(kn.makeEmpty(), kn.makeLvar("__fresh_0")),
				kn.eq(kn.makeLvar("qq"), kn.makeLvar("__fresh_2")),
				kn.either(
					kn.all(
						cons(
							kn.makeLiteral("1"),
						...
						kn.eq(kn.makeEmpty(), kn.makeLvar("__fresh_3")),
						appendo(
							kn.makeLvar("mid"),
							kn.makeLvar("__fresh_4"),
							kn.makeLvar("__fresh_5"),
				...
				return kn.either(
					...
					kn.fresh3((a, d, res) =>
						kn.all(
							...
							kn.eq(kn.makePair(a, res), o),
							// appendo(d, s, res)
							kn.apply_pred(kn.makelvar("wq"), d, s, res),
						),
					),
				)(scc);
			};
