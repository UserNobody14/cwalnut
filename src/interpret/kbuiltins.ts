...
const firsto = (a: kn.LTerm, l: kn.LTerm): kn.Goal => {
	...
	return kn.call_fresh((v) => kn.eq(kn.makePair(a, v), l));
};
const resto = (l: kn.LTerm, r: kn.LTerm): kn.Goal => {
	// console.log("resto", l.toString(), r.toString());
	return kn.call_fresh((v) => kn.eq(kn.makePair(v, r), l));
};
...
const appendo = (
	...
	// );
	return kn.either(
		kn.all(kn.eq(kn.makeEmpty(), l), kn.eq(s, o)),
		kn.call_fresh((a) =>
			kn.call_fresh((d) =>
				kn.call_fresh((res) =>
					kn.all(
...
const listToString = (l: kn.LTerm): string | null => {
	if (l instanceof kn.LPair) {
		if (
			l.first instanceof kn.LLiteral &&
			typeof l.first.value === "string"
		) {
...
const string_to_list = (
	...
	return (sc: kn.State): Iterable<kn.State> => {
		...
		if (
			ssf instanceof kn.LLiteral &&
			typeof ssf.value === "string"
		) {
			return kn.eq(
				l,
				kn.makeList(
					[...ssf.value].map((c) => new kn.LLiteral(c)),
				),
			)(sc);
		}
...
const addc = (
	...
	return (sc) => {
		...
		if (
			aWalked instanceof kn.LLiteral &&
			bWalked instanceof kn.LLiteral
		) {
			if (
				...
				return eq(
					c,
					kn.makeLiteral(aWalked.value + bWalked.value),
				)(sc);
			}
			if (
			...
			aWalked instanceof kn.LLiteral &&
			cWalked instanceof kn.LLiteral
		) {
			if (
				typeof aWalked.value === "number" &&
				...
				return eq(
					b,
					kn.makeLiteral(cWalked.value - aWalked.value),
				)(sc);
			}
			if (
				typeof aWalked.value === "string" &&
			...
			if (
				...
				return eq(
					b,
					kn.makeLiteral(
						cWalked.value.slice(aWalked.value),
					),
				)(sc);
			}
			if (
				...
				return eq(
					b,
					kn.makeLiteral(
						cWalked.value - aWalked.value.length,
					),
				)(sc);
			} else {
				throw new Error("Invalid types for add");
			...
			if (
				...
				return eq(
					a,
					kn.makeLiteral(cWalked.value - bWalked.value),
				)(sc);
			}
			if (
				typeof bWalked.value === "string" &&
			...
			if (
				...
				return eq(
					a,
					kn.makeLiteral(
						cWalked.value.slice(0, bWalked.value),
					),
				)(sc);
			}
			if (
				...
				return eq(
					a,
					kn.makeLiteral(
						cWalked.value - bWalked.value.length,
					),
				)(sc);
			} else {
				throw new Error("Invalid types for add");
...
type BuiltinRecord = Record<
	Builtin,
	LPredicate | LPredicateFn
>;
// const defaultPred: LPredicateFn = () => {
//     throw new Error("Not implemented");
// };
const defaultPred: (s: string) => LPredicateFn =
	(s: string) => () => {
...
const toBuiltinInclusionGoal = () => {
	...
	return kn.all(
		...Object.entries(builtins2).map(([name, fn]) => {
			return kn.eq(
				kn.makelvar(name),
				typeof fn === "function"
					? new LPredicate(name, fn)
					: fn,
			);
		}),
	);
