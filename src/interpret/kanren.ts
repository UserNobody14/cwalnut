...
function* mergeStreams(
	state: State,
	functions: Array<(state: State) => MatureStream>,
): MatureStream {
...
function* mapStreams(
	goal: (s: State) => Iterable<State>,
	stream: Iterable<State>,
): MatureStream {
...
export class LPair extends LTerm {
	...
	public doesOccur(
		...
		return (
			find(this.first).doesOccur(x, find) ||
			find(this.second).doesOccur(x, find)
		);
	}
	public nonEquivalentUnite(
		s: Subst,
...
export class LFunctor extends LTerm {
	...
	public nonEquivalentUnite(
		...
		const sz = this.args.reduce<Subst | null>(
			(acc, arg, i) => {
...
> extends LTerm {
	...
	doesOccur(x: string, find: (z: LTerm) => LTerm): boolean {
		return (
			this.term.doesOccur(x, find) ||
			this.constraintArgs.some((arg) =>
				arg.doesOccur(x, find),
			)
		);
	}
...
export class Subst extends ImmRecord(substDefaults) {
	public extend(x: LLVar, v: LTerm): Subst {
		...
		if (v.doesOccur(x.name, (z) => this.find(z)))
			return this;
...
export class ConstrainL {
	constructor(
		...
		public constraintKind: string,
		public constraintArgs: LTerm[],
	) {}
	apply(s: State): Iterable<State> | State {
		return this.constraint(
			this.constraintArgs.map((arg) => arg.reify(s.subst)),
			s,
		);
	}
}
...
export class State extends ImmRecord(stateDefaults) {
	toMap(
		...
		return this.subst.pairs.reduce(
			(acc, spair) => {
				const v = spair.first;
				...
				if (v instanceof LLVar) {
					...
					if (
						seekvars &&
						vars2seek.some((v2) => v2.name === v.name)
					) {
...
export function fresh2(
	f: (v: LLVar, v2: LLVar) => Goal,
): Goal {
	return call_fresh((v) => call_fresh((v2) => f(v, v2)));
}

export function fresh3(
	f: (v: LLVar, v2: LLVar, v3: LLVar) => Goal,
): Goal {
	return call_fresh((v) =>
		fresh2((v2, v3) => f(v, v2, v3)),
	);
}

export function fresh4(
	f: (v: LLVar, v2: LLVar, v3: LLVar, v4: LLVar) => Goal,
): Goal {
	return call_fresh((v) =>
		fresh3((v2, v3, v4) => f(v, v2, v3, v4)),
	);
}
...
export function freshNom(fn: (v: LNom) => Goal): Goal {
	return (sc) => {
