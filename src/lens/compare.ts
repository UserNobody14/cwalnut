...
export function captureEquality<T>(
	a1: TermGeneric<T>,
	b1: TermGeneric<T>,
): boolean {
	if (a1.type !== b1.type) {
	...
	return match([a1, b1])
		...
		.with(matchfn("fresh"), ([a, b]) => {
			...
			return a.newVars.every(
				(v, i) => v.value === b.newVars[i].value,
			);
		})
		.with(matchfn("with"), ([a, b]) => {
			return (
				a.name.value === b.name.value &&
				captureEquality(a.body, b.body)
			);
		})
		...
		.with(
			...
			([a, b]) => {
				if (a.type !== b.type) {
					...
					console.warn(
						"Unknown equality fail potentially",
						a,
						b,
					);
					return false;
