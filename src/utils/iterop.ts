function* mergeStreams<VALUE>(
	state: VALUE,
	functions: Array<(state: VALUE) => Iterable<VALUE>>,
): Iterable<VALUE> {
	// Initialize an array of iterators from the functions
	const iterators = functions.map((func) =>
		func(state)[Symbol.iterator](),
	);
	const queue: Array<Iterator<VALUE>> = [...iterators];
	yield* mergeGeneral(queue);
}

export function* mergeGeneralIterable<VALUE>(
	iterators: Array<Iterable<VALUE>>,
): Iterable<VALUE> {
	const queue: Array<Iterator<VALUE>> = [
		...iterators.map((iterable) =>
...
export function* mergeGeneral<VALUE>(
	queue: Array<Iterator<VALUE>>,
): Iterable<VALUE> {
	while (queue.length > 0) {
		const iterator = queue.shift();
		if (iterator) {
...
export function* mapStreams<VALUE, RES>(
	goal: (s: VALUE) => Iterable<RES>,
...
export function* reduceStreams1<VALUE, ACC>(
	goal: (s: VALUE) => Iterable<VALUE>,
	stream: Iterable<VALUE>,
	reducer: (acc: ACC, val: VALUE) => ACC,
	initial: ACC,
): Iterable<ACC> {
	let acc = initial;
...
export function* reduceStreamArray<VALUE>(
	worlds: VALUE[],
	reducer: (acc: VALUE, val: VALUE) => Iterable<VALUE>,
): Iterable<VALUE> {
	// let acc = arr[0];
	...
	const pairs = mergePossibilities([
		leftStream,
		rightStream,
	]);
	yield* mapStreams(
		(pair) => reducer(pair[0], pair[1]),
		pairs,
	);
...
export function* mergePossibilities<VALUE>(
	queue: Array<Iterable<VALUE>>,
): Iterable<Array<VALUE>> {
	// Initialize an array of iterators from the functions
	const iterators = queue.map((iterable) =>
		iterable[Symbol.iterator](),
	);
...
export function* mergePossibilitiesGeneral<VALUE>(
	possibilityGen: Array<Iterator<VALUE>>,
): Iterable<Array<VALUE>> {
...
function* mapPossibilitiesGeneral<VALUE>(
	value: VALUE,
	rest: Array<Iterator<VALUE>>,
): Iterable<Array<VALUE>> {
	// Breadth first merger & mapping of the streams
	const queue: Array<Iterator<VALUE>> = [];
	let streamDone = false;
...
export function* takeToIterable<VALUE>(
