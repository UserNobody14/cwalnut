import { IdentifierDsAst, LiteralDsAst, TermDsAst } from "src/types/DesugaredAst";
...
export class ImmutableAstEnv {
    ...
    cloneEnv(addTerms: TermDsAst[], addLvars: IdentifierDsAst[], lvarCounter: number): ImmutableAstEnv {
        const newEnv = new ImmutableAstEnv();
        newEnv.lvarCounter = this.lvarCounter + lvarCounter;
        newEnv.currentlyScopedLvars = [...this.currentlyScopedLvars, ...addLvars];
        newEnv.currentlyOperatingTerms = [...this.currentlyOperatingTerms, ...addTerms];
        return newEnv;
    }
    ...
    performOp<T>(op: (env: ImmutableAstEnv) => [T, ImmutableAstEnv]): T {
        return op(this)[0];
    }

    onSubEnv<T>(op: (env: ImmutableAstEnv) => ImmutableAstEnv): ImmutableAstEnv {
        const newEnv = new ImmutableAstEnv();
        newEnv.currentlyOperatingTerms = [];
        newEnv.currentlyScopedLvars = [...this.currentlyScopedLvars];
        return op(newEnv);
    }

    monadic1<T>(op: (env: [T, ImmutableAstEnv]) => [T, ImmutableAstEnv], list: T[]): [T[], ImmutableAstEnv] {
        return list.reduce<[T[], ImmutableAstEnv]>(([acc, env], item) => {
