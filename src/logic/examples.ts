...
const outv = kn.run1(
	...
	kn.all(
		...
		appendo(qlvar.einput, qlvar.__fresh_1, qlvar.__fresh_2),
		appendo(qlvar.input2, qlvar.__fresh_0, qlvar.__fresh_1),
				...
				eq(
					qlam(
						ezTie(
							...
							qapp(
								qlam(
									ezTie(x, qapp(qvar(f), qapp(qvar(x), E))),
								),
								qlam(ezTie(x, qapp(qvar(f), qappVV(x, x)))),
							),
						),
