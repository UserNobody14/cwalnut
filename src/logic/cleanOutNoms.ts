...
function isTieObject(
	obj: CleanOutput,
): obj is { name: string; term: CleanOutput } {
	return (
		typeof obj === "object" &&
		"name" in obj &&
		"term" in obj
...
function isSwapObject(
	obj: CleanOutput,
): obj is { swap: [string, string]; term: CleanOutput } {
	return (
		typeof obj === "object" &&
		"term" in obj &&
		"swap" in obj
...
export function cleanOutNoms1(
	c: CleanOutput,
	carryThrough: CarryThrough,
): [CleanOutput, CarryThrough] {
	return match<CleanOutput, [CleanOutput, CarryThrough]>(c)
		.when(isTieObject, (tie) => {
			const [nomN, carryThrough2] = cleanOutNoms1(
				tie.name,
				carryThrough,
			);
			const [termN, carryThrough3] = cleanOutNoms1(
				tie.term,
				carryThrough2,
			);
			return [
		...
		.when(isSwapObject, (nom) => {
			const [nomN1, carryThrough2] = cleanOutNoms1(
				nom.swap[0],
				carryThrough,
			);
			const [nomN2, carryThrough3] = cleanOutNoms1(
				nom.swap[1],
				carryThrough2,
			);
			const [termN, carryThrough4] = cleanOutNoms1(
				nom.term,
				carryThrough3,
			);
			return [
		...
		.when(
			(arr: CleanOutput): arr is CleanOutput[] =>
				Array.isArray(arr) && !(typeof arr === "string"),
			(arr) => {
				const [newArr, carryThrough2] = arr.reduce<
					[CleanOutput[], CarryThrough]
				>(
					([acc, carryThrough], item) => {
						const [newItem, carryThrough2] = cleanOutNoms1(
							item,
							carryThrough,
						);
		...
		.when(
			...
			(nom: CleanOutput): nom is string =>
				typeof nom === "string" && nom.startsWith("Nom("),
		...
		.when(
			(str: CleanOutput): str is string =>
				typeof str === "string",
			(str) => [str, carryThrough],
		)
		.when(
			(num: CleanOutput): num is number =>
				typeof num === "number",
			(num) => [num, carryThrough],
		)
		.when(
			(bool: CleanOutput): bool is boolean =>
				typeof bool === "boolean",
