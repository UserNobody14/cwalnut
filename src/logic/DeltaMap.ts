import { Map as ImmMap, Set as ImmSet, Record as ImmRecord } from "immutable";
...
export class DeltaMap extends ImmRecord(deltaDefaults) {
    ...
    public setOneVarAndName(name: LNom, lvar: LLVar): DeltaMap {
        // First add the name to the varToNames map
        const varToNames2 = this.varToNames.get(lvar.name) ?? ImmSet();
        const varToNames3 = varToNames2.add(name.name);
        const varToNames4 = this.varToNames.set(lvar.name, varToNames3);
        // Now add the var to the nameToVar map
        const nameToVars2 = this.nameToVars.get(name.name) ?? ImmSet();
        const nameToVars3 = nameToVars2.add(lvar.name);
        const nameToVars4 = this.nameToVars.set(name.name, nameToVars3);
        return this.set('nameToVars', nameToVars4).set('varToNames', varToNames4);
    }
    public restrict(names: ImmSet<LNom | string> | (LNom | string)[], ...lvars: LLVar[]): DeltaMap {
