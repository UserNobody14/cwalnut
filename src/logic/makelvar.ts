...
import { LLVar, LLiteral, LEmpty, LNom, type LTerm, LPair, LTie } from "./terms";
...
export const qlvar: Record<string, LLVar> =
new Proxy(
    {},
    {
...
export const makeLNom = (name: string): LNom => new LNom(name);
export const qnom: Record<string, LNom> =
new Proxy(
    {},
    {
...
export const makeList = (list: LTerm[]): LPair | LTerm => {
    ...
    if (list.length === 1) return makePair(list[0], makeEmpty());
...
export const makeTie = (name: LNom, term: LTerm): LTie => new LTie(name, term);