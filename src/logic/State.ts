import { List as ImmList, Map as ImmMap, Record as ImmRecord } from "immutable";
import {type LTerm, LLVar, LNom} from './terms';
import { Subst } from "./Subst";
import {type CleanOutput, type Nominal, type ImmatureStream, StreamFailed, type SingletonStream} from './types';
...
export class State extends ImmRecord(stateDefaults) {
    ...
    toMap(showInternals = false, vars2seek1: string[] | LLVar[] = []): { [k: string]: string; } {
        ...
        const clo = this.subst.toClean(showInternals, vars2seek);
    ...
    toClean(showInternals = false, vars2seek1: string[] | LLVar[] = []): CleanOutput {
        ...
        return this.subst.mp.reduce((acc, spairsecond, spairfirst) => {
            const v = spairfirst;
                    ...
                    acc[v] = val.map(
                        (valT) => {
                            ...
                            if (valT instanceof LLVar) {
                                ...
                                const fenvCheck = this.i.nominal.fenv.get(valT.name);
                                const fenv2Check = this.i.nominal.fenv2.get(valT.name);
                                if (fenvCheck) {
                                    // acc[v] = `FENV1_SUSPENSION__${fenvCheck.toString()}`;
                                    return new LLVar(`FENV1_SUSPENSION__${fenvCheck.toString()}`);
                                } else if (fenv2Check) {
                                    return new LLVar(`FENV2_SUSPENSION__${fenv2Check.toString()}`);
                                } else {
                                    return valT;
                                }
                            } else {
