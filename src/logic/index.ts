...
import type { AnyGoal, CleanOutput, SingletonGoal } from "./types";
import { emptyState, type State } from "./State";
import { topLevelUnificationS } from "./topLevelUnification";
import { makelvar } from "./makelvar";
import { takeT, allM, eitherM, failed, type MGoal } from "./streams";
import { type CarryThrough, cleanOutNoms1 } from "./cleanOutNoms";
...
export function run(n: number | null, g: MGoal, extraNum?: number): State[] {
    if (n === null) {
...
export function run1(
    ...
    return runProject(n, g, (s) => {
        const out: Record<string, CleanOutput> = {};
        let carryThrough: CarryThrough = { noms: [], lvars: [] };
        for (const vname of v) {
            const val = makelvar(vname).reify(s.subst);
            if (val) {
                const [vCl, ct2] = cleanOutNoms1(val.cleanOutput(), carryThrough);
                carryThrough = ct2;
                out[vname] = vCl;
            }
        }
...
export function apply_pred(
    ...
    return (sc) => {
        ...
        const arggs = lpp.fn(...args.map((arg) => sc.reify(arg)));
