...
export const qtyping = (t1: LTerm, t2: LTerm) =>
	makeList([makeLiteral("->"), makeList([t1, t2])]);
export const qmc = (t1: LTerm) =>
	makeList([makeLiteral("C"), t1]);
export const qcleanOutput = (
	t1: LTerm | CleanOutput,
): CleanOutput => {
	return t1 instanceof LTerm ? t1.cleanOutput() : t1;
};
export const qtyping2 = (
	t1: LTerm | CleanOutput,
	t2: LTerm | CleanOutput,
): CleanOutput => {
...
export function typo(g: LTerm, e: LTerm, te: LTerm): MGoal {
	return all(
		either(
			fresh((x) => all(eq(qvar(x), e), lookupo(x, te, g))),
			fresh4((rator, trator, rand, trand) =>
				all(
...
export function findModes(
	modes: LTerm,
	ovars: LTerm,
	stateLookupBefore: LTerm,
	stateLookupAfter: LTerm,
): MGoal {
...
export function inferModesForVars(
	predApp: LTerm,
	defLookup: LTerm,
	stateLookupBefore: LTerm,
	stateLookupAfter: LTerm,
): MGoal {
	return fresh2((ovars, predName) =>
		all(
			eq(qpredAppI(predName, ovars), predApp),
			fresh4((modes, outDef, fnDef, fnDet) =>
				all(
...
export function inferStatesForPredApps(
	predApps: LTerm,
	predDefs: LTerm,
	stateLookupBefore: LTerm,
	stateLookupAfter: LTerm,
	...
	return either(
		...
		fresh4((predApp, stateAfterHat, predAppsTail) =>
			all(
...
export function inferModesForPredApps(
	predApps: LTerm,
	modes: LTerm,
	ovars: LTerm,
	predDefs: LTerm,
...
export function unifyDisjunction(
	disjunct1: LTerm,
	disjunct2: LTerm,
	predDefs: LTerm,
	ovars: LTerm,
	...
	return fresh6(
		(modes1, modes2, stB41, stA1, stB42, stA2) =>
			all(
			...
			fresh4(
				(predApps, stB4Adjusted, stAAdjusted, outType) =>
					all(
						...
						eq(
							makePair(makePair(fr, outType), stA),
							stAAdjusted,
						),
						inferStates(
							predApps,
	...
	return either(
		...
		fresh6(
			(
				...
				stateAfter,
				ovarsTail,
				stateLookupBeforeTail,
				stateLookupAfterTail,
			) =>
				all(
					eq(ovars, makePair(ovar, ovarsTail)),
					eq(
						stateLookupBefore,
						makePair(stateBefore, stateLookupBeforeTail),
					),
					eq(
						stateLookupAfter,
						makePair(stateAfter, stateLookupAfterTail),
					),
					// Set the noms to an empty variable
					fresh2((emptyState1, emptyState2) =>
						all(
...
export function inferDisjunction1(
	disjunct: LTerm,
	ovars: LTerm,
	modes: LTerm,
	predDefsIn: LTerm,
...
export function inferDisjunction(
	disjunct: LTerm,
	ovars: LTerm,
	modes: LTerm,
	predDefsIn: LTerm,
