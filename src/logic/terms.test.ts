...
import { type LLVar, LNom, LSuspension, type LTerm } from "./terms";
import { makeList, makeLiteral, makeLvar, makeTie, qlvar, qnom } from "./makelvar";
import { emptyState, type State } from "./State";
import { ezIn } from "./ezeq";


function emptyUnify0(
    a: LTerm,
    b: LTerm
) {
...
function emptyUnify1(
    a: LTerm,
    b: LTerm
) {
    const uu = emptyState.unify(a, qlvar.result)?.unify(b, qlvar.result);
    if (!uu) return null;
...
function emptyUnifyPermute1(
    ...
    return (s: State) => left === 'left' ? s.unify(a, qlvar.result) : s.unify(qlvar.result, a);
}
...
function emptyUnify(
    ...
    return emptyUnifyPermute2(a, aleft, b, bleft)?.reify(qlvar.result) ?? null;
}
...
function dispUnify(
    ...
    const a = swapTerms === 'swap' ? b1 : a1;
    const b = swapTerms === 'swap' ? a1 : b1;
    const aString = aleft === 'left' ? `${a.toString()} = V` : `V = ${a.toString()}`;
    const bString = bleft === 'left' ? `${b.toString()} = V` : `V = ${b.toString()}`;
    return `[${aString}] ;; [${bString}]`;
}


function* swapSets(): Iterable<['swap' | 'noswap', 'left' | 'right', 'left' | 'right']> {
...
function makeSusp(a: [string | LNom, string | LNom][], b: LLVar): LSuspension | LLVar {
    if (a.length === 0) {
        return b;
    }
    ...
    return new LSuspension(
        ...
        [af1 instanceof LNom ? af1 : new LNom(af1), af2 instanceof LNom ? af2 : new LNom(af2)]
        , makeSusp(aRest, b));
}
...
describe("terms", () =>{
    ...
    it("More complex", () => {
        ...
        expect(foundItem?.reify(example2.subst).cleanOutput()).toEqual(
            {
                name: "Nom(a)",
    ...
    it("Unifies Basic", () => {
        const exampleU = makeTie(qnom.a, makeList([makeLiteral("foo"), qnom.a, makeLiteral(3), qlvar.b]));
        const example1 = emptyState.unify(exampleU, qlvar.t1);
        expect(example1).not.toBe(null);
        if (!example1) return;
        const foundItem = example1.find(qlvar.t1);
        expect(foundItem?.reify(example1.subst).cleanOutput()).toEqual(
            {
                name: "Nom(a)",
                term: ["foo", "Nom(a)", 3, "?b"],
            }
...
describe("Suspensions", () =>{
    ...
    it("Unifies Suspensions x LiteralsMINUS1", () => {
        ...
        const example1 = exampleU.unite(
            emptyState,
            exampleV
        );
        expect(example1).not.toBe(null);
        if (!example1) return;
        expect(example1.find(qlvar.A).toString()).toEqual(
            "B"
        );
    });
    ...
    it("Unifies Suspensions x LiteralsMINUS2", () => {
        ...
        expect(example1).not.toBe(null);
        if (!example1) return;
        expect(example1.find(qlvar.Z).toString()).toEqual(
            "B"
        );
    });
    ...
    it("Unifies Suspensions x LiteralsMINUS3ddd", () => {
        ...
        expect(example1).not.toBe(null);
        if (!example1) return;
        expect(example1.reify(qlvar.Z).toString()).toEqual(
            "B"
        );
    });
    ...
    it("Unifies Suspensions x LiteralsMINUS4", () => {
        ...
        expect(example1).not.toBe(null);
        if (!example1) return;
        console.log("EXAMPLE1LOCAL", JSON.stringify(example1.toClean(), null, 2));
        ...
        expect(example2).not.toBe(null);
        if (!example2) return;
        console.log("EXAMPLE2LOCAL", JSON.stringify(example2.toClean(), null, 2));
        expect(example2.reify(qlvar.A).toString()).toEqual(
            "B"
        );
    });
    ...
    it("Unifies Suspensions x Literals1", () => {
        ...
        for (const [swapTerms, aleft, bleft] of swapSets()) {
            const example1 = emptyUnify(exampleU, aleft, exampleV, bleft, swapTerms);
            expect(example1).not.toBe(null);
            if (!example1) return;
            //`Failed on ${dispUnify(exampleU, aleft, exampleV, bleft, swapTerms)}`
            
            expect(example1.cleanOutput()
        ).toEqual(
                "B"
            );
        }
    ...
    it("Unifies Suspensions x Ties2", () => {
        ...
        expect(example1).not.toBe(null);
        if (!example1) return;
        expect(qlvar.A.reify(example1.subst).cleanOutput()).toEqual(
            {
                name: "Nom(b)",
                term: ["Nom(b)", "Nom(a)"],
            }
...
describe("Suspensions2", () =>{
    ...
    it("Swaps correctly", () => {
        const exampleU = makeSusp([[qnom.a, qnom.b], [qnom.b, qnom.d]], qlvar.A);
...
describe("Ties", () =>{
        ...
        it("Unifies Ties x Ties0", () => {
            const exampleU = makeTie(qnom.a, makeList([makeLiteral("foo"), qnom.a, makeLiteral(3), qlvar.b]));
            const exampleV = makeTie(qnom.a, makeList([makeLiteral("foo"), qnom.a, makeLiteral(3), qlvar.b]));
            const example1 = emptyUnify0(exampleU, exampleV);
            expect(example1).not.toBe(null);
            if (!example1) return;
            expect(example1.cleanOutput()).toEqual(
        ...
        it("Unifies Ties and Suspensions3", () => {
            const exampleU = makeTie(qnom.a, ezIn(["foo", qnom.a, 3, qnom.b]));
            const exampleV = makeSusp([[qnom.a, qnom.b], [qnom.c, qnom.b]], qlvar.A);
