import type { Map as ImmMap, List as ImmList } from "immutable";
...
export type LPackage = [ImmList<[LTerm, LTerm]>, Subst, NablaList, number];

export type CombinedStateStreamType = State | StreamFailed | MatureStream | ImmatureStream;

export type ImmatureStream = Iterable<CombinedStateStreamType>;
...
export type Goal2 = (sc: State) => (MatureStream | ImmatureStream);
export type AnyGoal = Goal2 | SingletonGoal;
export type CleanOutput = string | number | boolean | { [k: string]: CleanOutput; } | CleanOutput[];
