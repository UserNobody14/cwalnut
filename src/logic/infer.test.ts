...
import { fresh6, freshNom2, freshNom4, freshNom6 } from "./AnyFreshFn";
import { qtyping, qtyping2, typo, findModes, qmode, inferDisjunction, qdisj, qpredApp, qpredDef, qpredDef1, inferStates, inferModesForVars, lookupPredDef, inferDisjunction1 } from './infer';
import { makeEmpty, makeList, makeList2, makeLiteral, makePair, qlvar, qnom } from './makelvar';
import { lamTie, lamTie2, qapp, qapp2, qvar } from "./testutils";
                ...
                freshNom2(
                    (c, d) => {
                        const defsIn = qpredDef(
                            makePair(c, qpredDef1(
                                makeList([qmode(makeLiteral("in"), makeLiteral("out"))]),
                                qlvar.unk1,
                                qlvar.unk2
                            )),
                            makePair(d, qpredDef1(
                                makeList([qmode(makeLiteral("out"), makeLiteral("out")), qmode(makeLiteral("in"), makeLiteral("out"))]),
                                qlvar.unk3,
                                qlvar.unk4
                            ))
                        );
        ...
        expect(
            run1(
                ...
                freshNom4(
                    (unifyo, membero, a) => {
                        const defsIn = qpredDef(
                            makePair(unifyo, qpredDef1(
                                makeList([qmode(makeLiteral("in"), makeLiteral("out"))]),
                                qlvar.unk1,
                                qlvar.unk2
                            )),
                            makePair(membero, qpredDef1(
                                makeList([qmode(makeLiteral("out"), makeLiteral("out")),
                                qmode(makeLiteral("in"), makeLiteral("out"))]),
                                qlvar.unk3,
                                qlvar.unk4
                            ))
