...
export abstract class LTerm {
    ...
    public abstract doesLvarOccur(x: string, find: (z: LTerm) => LTerm): boolean;
    public map(f: (z: LTerm, original?: LTerm) => LTerm): LTerm {
        return f(this, this);
    }
    public abstract nonEquivalentUnite(s: State, v: LTerm): State | null;
...
export class LLVar extends LTerm {
    ...
    public doesLvarOccur(x: string, _find: (z: LTerm) => LTerm): boolean {
        return this.name === x;
    }
    public nonEquivalentUnite(s: State, v: LTerm): State | null {
        throw new Error("LVars do not unite");
    }

    override varUnifyEmptyScope(s: State, v: LLVar): State | null {
...
export class LLiteral extends LTerm {
    ...
    public doesLvarOccur(_x: string, _find: (z: LTerm) => LTerm): boolean {
        return false;
    }
    public nonEquivalentUnite(s: State, v: LTerm): State | null {
...
export class LEmpty extends LTerm {
    ...
    public doesLvarOccur(x: string, find: (z: LTerm) => LTerm): boolean {
        return false;
    }

    public nonEquivalentUnite(s: State, v: LTerm): State | null {
        if (v instanceof LEmpty) return s;
        this.warn(`Failed to unite (Lempty) ${this.toString()} and ${v.toString()}`);
        return null;
    }
...
export class LPair extends LTerm {
    ...
    constructor(public readonly first: LTerm, public readonly second: LTerm) {
        super();
    }

    public isLiteral(): boolean {
        return this.first.isLiteral() && this.second.isLiteral();
    }
    toString(): string {
    ...
    map(f: (z: LTerm, orig?: LTerm) => LTerm): LTerm {
        return f(new LPair(this.first.map(f), this.second.map(f)), this);
    }
    ...
    public occursCheck(v: LTerm): boolean {
        return this.selfEquiv(v) || this.first.occursCheck(v) || this.second.occursCheck(v);
    }
    public doesLvarOccur(x: string, find: (z: LTerm) => LTerm): boolean {
        return find(this.first).doesLvarOccur(x, find) || find(this.second).doesLvarOccur(x, find);
    }
    public nonEquivalentUnite(s: State, v: LTerm): State | null {
        ...
        this.warn(`Failed to unite higher order lpair ${this.reify(s.subst).toString()} and ${v.reify(s.subst).toString()}`);
        return null;
    }
    // public varUnifyEmptyScope(s: State, v: LLVar): State | null {
    ...
    public varUnifyHigherOrder(s: State, t1: LLVar): State | null {
        // return s.withLvar((v1) => (sc) => {
        //     return sc.withLvar((v2) => (sc2) => {
        //         const pairVariable = new LPair(v1, v2);
...
export class LFunctor extends LTerm {
    ...
    public occursCheck(v: LTerm): boolean {
        return this.selfEquiv(v) || this.args.some((arg) => arg.occursCheck(v));
    }

    public doesLvarOccur(x: string, find: (z: LTerm) => LTerm): boolean {
        return this.args.every(arg => find(arg).doesLvarOccur(x, find));
    }
    public nonEquivalentUnite(s: State, v: LTerm): State | null {
        throw new Error("Method not implemented for now.");
...
export class LNom extends LTerm {
    ...
    public nonEquivalentUnite(s: State, v: LTerm): State | null {
        ...
        this.warn(`Failed to unite higher order ${this.toString()} and ${v.toString()}`);
        return null;
    }
     ...
     *           [(and (name? t₁) (variable? t₂))
           (let ([n₂ (gen-name φ₁ t₁ φ₂)])
             (and n₂
                  (list (ext-subst σ t₂ n₂) χ (cons t₂ xs))))]
     */
    public varUnifyHigherOrder(s: State, v: LLVar): State | null {
        throw new Error("Method not implemented for now.");
    }
...
export class LTie extends LTerm {
    type = 'tie' as const;
    public leftRank = 2;
    public readonly leftExceptions: Set<string> = new Set(['lvar']);
    ...
    map(f: (z: LTerm) => LTerm): LTerm {
        return f(new LTie(this.name.map(f) as LNom, this.term.map(f)));
    }
    equiv(v: LTie): boolean {
        const selfEquivA = this.name.selfEquiv(v.name) && this.term.selfEquiv(v.term);
        // const selfEquivB = (
...
export class LPredicate extends LTerm {
    ...
    public leftRank = 6;
    constructor(public readonly name: string, public readonly fn: LPredicateFn) {
        super();
    }
    public isLiteral(): boolean {
        return false;
...
export class LConstraint<T extends LTerm, Z extends LTerm> extends LTerm {
    ...
    constructor(
        ...
        public constraint: (s: State, curr: T, args: Z[]) => State | null
    ) {
        super();
    }
    ...
    public occursCheck(v: LTerm): boolean {
        return this.selfEquiv(v) || this.term.occursCheck(v) || this.constraintArgs.some((arg) => arg.occursCheck(v));
    }

    doesLvarOccur(x: string, find: (z: LTerm) => LTerm): boolean {
        return this.term.doesLvarOccur(x, find) || this.constraintArgs.some((arg) => arg.doesLvarOccur(x, find));
    }
...
export class LSuspension extends LTerm {
    ...
    public map(f: (z: LTerm, orig?: LTerm) => LTerm): LTerm {
        ...
        if (mappedTerm instanceof LSuspension) {
            return f(new LSuspension([this.swap[0], this.swap[1]], mappedTerm), this);
    ...
    public reify(_s: Subst): LTerm {
        ...
        if (rT instanceof LLVar) return this.formSuspension(this.toSwapList(), rT);
        if (rT instanceof LSuspension) return this.formSuspension([...this.toSwapList(), ...rT.toSwapList()], rT.getLvar());
        return this.applySwaps(rT);
    }

    formSuspension(s: [LNom, LNom][], v: LLVar): LSuspension | LLVar {
        if (s.length === 0) return v;
        return new LSuspension(s[0], this.formSuspension(s.slice(1), v));    
    }
    ...
    toSwapList(): [LNom, LNom][] {
        if (this.term instanceof LSuspension) return [[this.swap[0], this.swap[1]], ...this.term.toSwapList()];
        return [this.swap];
    }
    ...
    replaceLvar(v: LLVar): LLVar | LSuspension {
        if (this.term instanceof LLVar) return new LSuspension(this.swap, v);
        return new LSuspension(this.swap, this.term.replaceLvar(v));
    }
    ...
    private applySwapsToTerm(swaps: [LNom, LNom][], v: LTerm): LTerm {
        return swaps.reduceRight((acc, [a, b]) => applySwap([a, b], acc), v);
    }
...
(append (apply append π) (apply append ˆπ))))))
    ...
    disagreementSet(v: LSuspension | LLVar): Set<string> {
        ...
        for (const nom of allNoms) {
            const thisNom = this.applySwaps(new LNom(nom)) as LNom;
    ...
    private getCombinedSwaps(v: LSuspension | LLVar) {
        ...
        const thisSwapSet = new Set(thisSwapList.flat().map((a) => a.name));
        if (v instanceof LLVar) return thisSwapSet;
        const vSwapList = v.toSwapList();
        const vSwapSet = new Set(vSwapList.flat().map((a) => a.name));
    ...
    nonEquivalentUnite(s: State, v1: LTerm): State | null {
        ...
        if (v instanceof LSuspension) {
            if (this.swap[0].selfEquiv(v.swap[0]) && this.swap[1].selfEquiv(v.swap[1])) {
                return s.unify(this.term, v.term);
            } else if (this.swap[1].selfEquiv(v.swap[0]) && this.swap[0].selfEquiv(v.swap[1])) {
                // return s.unify(this.term, v.term);
                return s.unify(this.term, v.term);
            } 
             
            if (v.getLvar().selfEquiv(this.getLvar())) {
                throw new Error("Unification of two suspensions with the same lvar");
                ...
                //     // );
                // });
                return s.extend(this.getLvar(), this.applySwapsReversed(v));
            }
        }
    ...
    toString(): string {
        return `Susp([${
            this.toSwapList().map(([a, b]) => `${a.toString()} -> ${b.toString()}`).join(', ')
        }] ${this.getLvar().toString()})`;
    }
    ...
    public cleanOutput(): CleanOutput {
        return {
            swap: [this.swap[0].cleanOutput(), this.swap[1].cleanOutput()],
            term: this.term.cleanOutput()
        };
    }
}
...
export const pmtch = {
    ...
    lvarOrSusp: P.union(P.instanceOf(LLVar), P.instanceOf(LSuspension)),
    other: P.any
  };