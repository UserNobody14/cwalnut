...
export function availableo(
	...
	return (state: State): MStream => {
		...
		if (uh) {
			...
			// return StreamFailed.StreamOver;
			console.log(
				`availableo failed for ${n.toString()} ${t.toString()} ${src}`,
			);
...
export function hash(
	...
	return (state: State): MStream => {
		...
		if (n1 instanceof LLVar) {
			...
			console.log(
				`hash failed extra ${n1.toString()} ${t.toString()} ${src}`,
			);
			return [];
		} else {
			n = n1;
		...
		if (uh) {
			...
			// return StreamFailed.StreamOver;
			console.log(
				`Hash failed for ${n.toString()} ${t.toString()} ${src}`,
			);
			// throw new Error(`availableo failed for ${n.toString()} ${t.toString()} ${src}`);
			return [];
		}
