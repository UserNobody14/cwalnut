...
const qappVV = (a: trm.LTerm, b: trm.LTerm) =>
	qapp(qvar(a), qvar(b));
...
describe("checktk", () => {
	...
	test("Lvars properly handled", () => {
		const output = run1(
			...
			["x"],
			fresh((x) => {
				return eq(x, makeList([makeLiteral("a"), x]));
			}),
		);
		...
		const output2 = run1(
			...
			["x"],
			fresh((x) => {
				return eq(makeList([makeLiteral("a"), x]), x);
			}),
		);
	...
	test("Logic 38", () => {
		...
		expect(ooo16[0]).toEqual({
			q: [tieTag(a0, qvar(a0)), "?$&0", "?$&0"],
		});
		expect(ooo16[1]).toEqual({
			q: [tieTag(a0, qvar2("?$&0")), "?$&1", qvar2("?$&0")],
		});
     ...
     */
		expect(ooo16[3]).toEqual({
			q: [
				tieTag(
					a0,
					qlam2(tieTag(a1, qvar2(suspTag(a2, a0, "?$&0")))),
				),
				"?$&1",
				// qapp2("?$&0", "?$&0"),
     ...
     */
		expect(ooo16[5]).toEqual({
			q: [
				tieTag(
					a0,
					qlam2(tieTag(a1, qlam2(tieTag(a2, qvar2(a0))))),
				),
				"?$&0",
				qlam2(tieTag(a1, qlam2(tieTag(a2, "?$&0")))),
			],
		});
		/**
     *          (((tie-tag a.0
     ...
     */
		expect(ooo16[7]).toEqual({
			q: [
				tieTag(
					a0,
					qapp2(qvar2(suspTag(a1, a0, "?$&0")), qvar(a0)),
				),
				"?$&1",
				qapp2(qvar2("?$&0"), "?$&1"),
     ...
     */
		expect(ooo16[9]).toEqual({
			q: [
				tieTag(
					a0,
					qlam2(tieTag(a1, qapp2(qvar2(a0), qvar2(a0)))),
				),
				"?$&0",
  ...
  '())
    */
			...
			run1(
				...
				freshNom2((a, b) => {
					return betao(
						ezIn([
							"app",
							["lam", ezTie(a, ["lam", ezTie(b, qvar(a))])],
							qvar(b),
						]),
						lamTie(b, qvar(b)),
					);
	...
	test("Logic 43", () => {
		...
		expect(output).toEqual([
			{
				q1: qapp2(qlam2(tieTag(a0, qvar(a0))), "?$&0"),
				q2: "?$&0",
			},
			{
				q1: qapp2(
					qlam2(tieTag(a0, qvar2(suspTag(a1, a0, "?$&0")))),
					"?$&1",
				),
				q2: qvar2("?$&0"),
			},
			{
				q1: qapp2(
					qlam2(tieTag(a0, qlam2(tieTag(a1, qvar2(a0))))),
					"?$&0",
				),
  ...
  */
	...
	test("Logic 52: Y combinator", () => {
		...
		expect(output).toEqual([
			{
				t: qapp2(
					qlam2(tieTag(a0, qapp(qvar(a1), qappVV(a0, a0)))),
				...
				q: [
					qapp2(
						qlam2(
							tieTag(
								a0,
								qapp(qvar(a1), qapp(qvar(a0), qvar(a0))),
							),
						),
						qlam2(
							tieTag(a2, qapp(qvar(a1), qappVV(a2, a2))),
						),
					),
					qapp2(
						qvar2("Nom(1)"),
	...
	test("Logic 55p: Y combinator", () => {
		...
		const output = run1(1, ["qa", "qb"], output1);
		expect(output).toHaveLength(1);
		expect(output).toEqual([
			{
				qa: qapp2(
