import {List as ImmList, Map as ImmMap, Record as ImmRecord, Set as ImmSet } from 'immutable';
import {LEmpty, LLiteral, LLVar, LNom, LPair, LSuspension, type LTerm, LTie, pmtch} from './terms';
import {Subst} from './Subst';
import type { LPackage, NablaList, NablaPackage } from './types';
...
function composeSubst(sigma: Subst, tau: Subst): Subst {
    const sigma2 = sigma.mp.map((v, _k) => {
    ...
    if (nTau.some(
      (v, k) => {
        return v instanceof LLVar && v.name === k ||
...
export function unify(
  ...
  return unifyHash(delta, composeSubst(sigma, sigmaHat), s.delta, s.set('number', inc));
}
...
export function unifyHash(
  ...
  const d4 = applyNablaRules(remapEqns(delta.concat(delta2), sigma));
  if (!d4) return null;
...
function applySubst(
  ...
  if (typeof uv !== 'object') throw new Error(`Not an object: ${uv}`);
  ...
  // console.warn("applySubst", uv.type);
    ...
    .with(pmtch.pair, (uv) => {
      return new LPair(applySubst(s, uv.first), applySubst(s, uv.second));
    })
    .otherwise(() => {
      return uv;
    });
...
export function deltaToNabla(
  delta: DeltaMap
): NablaList {
  return delta.varToNames.reduce<NablaList>((acc, names, vr) => {
    const listOfNames = names.toArray();
    const lvar = new LLVar(vr);
    return acc.unshift(...listOfNames.map(
      (name): [LNom, LLVar] => {
...
export function applyDeltaNablaRules(
  ...
  if (delta.varToNames.size === 0) {
    ...
    const nablaRules = remapEqns<LNom>(deltaToNabla(delta), s.subst);
...
function nablaRules(
    ...
    if (typeof d !== 'object') throw new Error(`Not an object: ${d}`);

    return match<LTerm, NablaPackage | null>(d)
    .with(pmtch.nom, (d) => d.name === a.name,
      (_d) => null
    )
    .with(pmtch.nom, (d) => d.name !== a.name,
      (_d) => [delta, emptyDelta]
    )
    .with(pmtch.tie, (d) => d.name.name === a.name,
      (_d) => [delta, emptyDelta]
    )
    .with(pmtch.tie, (d) => d.name.name !== a.name,
      (d): NablaPackage => [delta.unshift([a, d.term]), emptyDelta]
    )
    .with(pmtch.susp, (d): NablaPackage | null => {
      ...
      if (a.name === d.swap[1].name) {
        return [delta.unshift([d.swap[0], d.term]), emptyDelta];
      } else if (a.name === d.swap[0].name) {
        return [delta.unshift([d.swap[1], d.term]), emptyDelta];
      } else {
    ...
    .with(pmtch.pair, (d): NablaPackage => {
      return [delta.unshift([a, d.first], [a, d.second]), emptyDelta];
    })
    .with(pmtch.literal, () => {
    ...
    .otherwise(() => {
      console.error("NablaRules", a.toString(), d.toString());
      return null;
    });
}
...
function applySigmaRules2(
  ...
  if (eqns.size === 0) return [emptySigma, emptyNabla, increment];
...
function sigmaRules(
  ...
  // eqns.map(([a, b]) => [a.type, b.type]).toArray());
    return match<[LTerm, LTerm], LPackage | null> (eqn)
    .with([pmtch.other, pmtch.other], ([a, b]) => {
    ...
    //   return [eqns, emptySigma, emptyNabla, increment];
    // })
    .with([pmtch.tie, pmtch.tie], ([a, b]) => {
      return a.name.name === b.name.name
    }, ([c, c2]): LPackage | null => {
      return [eqns.unshift([c.term, c2.term]), emptySigma, emptyNabla, increment];
    })
    .with([pmtch.tie, pmtch.tie], ([a, b]) => {
      return a.name.name !== b.name.name;
    }, ([a, b]): LPackage | null => {
      ...
      return [eqns.unshift([t, uHat]), emptySigma, ImmList([[aa, tHat]]), increment];
    })
    ...
    .with([pmtch.lvar, pmtch.notsusp], ([a, b]) => {
      return b.occursCheck(a);
    }, ([_a, _b]): LPackage | null => {
      console.log("sigmaRules lvar failing!!!", [eqn[0].toString(), eqn[1].toString()]);
      return null;
      // return [eqns, emptySigma, emptyNabla, increment];
    })
    .with([pmtch.notsusp, pmtch.lvar], ([a, b]) => {
    ...
    .with([pmtch.susp, pmtch.lvar], ([a, b]) => {
      return !a.occursCheck(b);
    },  ([a, b]):  LPackage | null => {
      return updateSuspension(a, b, eqns, increment);
      // return updateGeneral(a, b, eqns, increment);
    })
    .with([pmtch.lvar, pmtch.susp], ([a, b]) => {
      return !b.occursCheck(a);
    }, ([a, b]): LPackage | null => {
      return updateGeneral(b, a, eqns, increment);
    })
    .with([pmtch.lvarOrSusp, pmtch.lvarOrSusp], ([a, b]) => {
      return a.occursCheck(b) || b.occursCheck(a);
    }, ([c, c2]): LPackage | null => {
      return applyDisagreements(c, c2, eqns, increment);
    })
    .with([pmtch.susp, pmtch.other], ([a, b]) => {
      return !b.occursCheck(a);
    }, ([a, b]): LPackage | null => {
      return updateSuspension(a, b, eqns, increment);
    })
    .with([pmtch.other, pmtch.susp], ([a, b]) => {
      return !a.occursCheck(b);
    }, ([a, b]): LPackage | null => {
    ...
    .with([pmtch.pair, pmtch.pair], ([a, b]): LPackage | null => {
      // return [eqns, ImmMap(), ImmMap()];
      if (a.first.selfEquiv(b.first)) {
        if (a.second.selfEquiv(b.second)) {
          return [eqns, emptySigma, emptyNabla, increment];
        }
        return [eqns.unshift([a.second, b.second]), emptySigma, emptyNabla, increment];
      } else if (a.second.selfEquiv(b.second)) {
        return [eqns.unshift([a.first, b.first]), emptySigma, emptyNabla, increment];
      }
      return [eqns.unshift([a.first, b.first], [a.second, b.second]), emptySigma, emptyNabla, increment];
    })
    .otherwise(() => {
...
function updateSuspension(b: LSuspension, a: LTerm, eqns: ImmList<[LTerm, LTerm]>, increment: number): LPackage | null {
  ...
  // }
  return updateGeneral(b.applySwapsReversed(a), b.getLvar(), eqns, increment);
}
...
function updateGeneral(b: LTerm, a: LLVar, eqns: ImmList<[LTerm, LTerm]>, increment: number): LPackage | null {
...
function splitUpPairs(
  ...
  const res = ((aP.second instanceof LPair && bP.second instanceof LPair) ? splitUpPairs(aP.second, bP.second) : [[aP.second, bP.second] as [LTerm, LTerm]]);
...
function applyDisagreements(
  ...
  if (!dgSet || !dgSet.size) return [eqns, emptySigma, emptyNabla, increment];
  return [
    eqns, 
    emptySigma, 
    ImmList(
