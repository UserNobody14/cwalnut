...
import { countVarsInCalls, mapPredCalls } from "src/lens/into-vars";
...
describe("cleanupLinearUnifies", () => {
    test("returns terms unchanged with pass flag true", () => {
        const terms = [make.predicate_call(make.identifier("info", "test"), [])];
        const result = cleanupLinearUnifies(terms, ImmSet(), true);
