...
describe("linkDirection", () => {
	const { a, b, c, d } = make.lvar2(undefined);
	const { unknown } = make.pred2(undefined);
	test("a in prev and b in next", () => {
		const prev = [[unknown(a)]];
		const next = [[unknown(b)]];
		expect(linkDirection("a", "b", prev, next)).toEqual({
			first: "a",
			next: "b",
		});
	});
	...
	test("a in prev and b in next", () => {
		const prev = [[unknown(a)]];
		const next = [[unknown(b)]];
		expect(linkDirection("b", "a", prev, next)).toEqual({
			first: "a",
			next: "b",
		});
	});
