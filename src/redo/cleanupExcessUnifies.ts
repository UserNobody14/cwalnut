...
type UnificationData = [
	ImmMap<string, string>,
	ImmSet<string>,
];

export function simplifyUnifyChain<T>(
	terms: TermGeneric<T>,
): [TermGeneric<T>, UnificationData] {
	return splitAlongScope(terms, simplifyOneUnification, [
		ImmMap<string, string>(),
		ImmSet<string>(),
	] as const);
}
...
export function simplifyOneUnification<T>(
	...
	currCall: PredicateCallGeneric<T>,
	...
	if (
		...
		if (nlink) {
			const replacements3 = replacements.set(
				nlink.next,
				nlink.first,
			);
...
export function refactorTermsToMergeUnifies<T>(
	rs: TermGeneric<T>,
) {
	return mapConjunctions([rs], refactorUnifications<T>, []);
}
...
export function refactorUnifications<T>(
	...
	return [conjunction1(...newUnifies, ...notUnifies), zz];
}
export function filterOutUnifyCalls<T>(
	qterms: TermGeneric<T>[],
) {
	return qterms.filter(
		(t) =>
			!(
				t.type === "predicate_call" &&
				t.source.value === "unify"
			),
	);
}

export function filterUnifyPredicateCalls<T>(
	qterms: TermGeneric<T>[],
) {
	return qterms.filter(
		(t): t is PredicateCallGeneric<T> =>
			t.type === "predicate_call" &&
			t.source.value === "unify",
	);
}

export function groupUnifies<T>(
	allUnifies: PredicateCallGeneric<T>[],
) {
	let mergeTargets: ExpressionGeneric<T>[][] = [];
	let idvf: T | null = null;
	for (const unify of allUnifies) {
		if (unify.source.value !== "unify")
			throw new Error("Expected unify");
		...
		mergeTargets = connectMergeTargets(
			mergeTargets,
			unify.args,
		);
	}
	...
	for (const mt of mergeTargets) {
		const allLiterals = mt.filter(
			(m) => m.type === "literal",
		);
		const allIdentifiers = mt.filter(
			(m) => m.type === "identifier",
		);
		const uniqueVars = new Set(
			allIdentifiers.map((m) => m.value),
		);
		const uniqueIdentifiers = Array.from(uniqueVars)
			.sort(
			...
			.map((v) =>
				make.identifier(
					mt.find(
						(m): m is IdentifierGeneric<T> =>
							m.type === "identifier" && m.value === v,
					)?.info ?? idvf,
					v,
				),
			);
		newUnifies.push(
			make.predicate_call(make.identifier(idvf, "unify"), [
				...uniqueIdentifiers,
				...allLiterals,
			]),
		);
...
export function connectMergeTargets<T>(
	...
	const [mergeTargetsWith, mergeTargetsWithout] =
		splitUnificationSets(mergeTargets, args);
