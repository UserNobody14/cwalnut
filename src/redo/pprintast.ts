import { TermDsAst, ExpressionDsAst } from "src/types/DesugaredAst";
...
export function pprintDsAst(ast: TermDsAst | TermDsAst[]): string {
    ...
    switch (ast.type) {
        case 'conjunction':
            return `conj:
${ast.terms.map(pprintDsAst).map(ii => indentStr(1, ii)).join('\n')}`;
        case 'disjunction':
            return `disj:
${ast.terms.map(pprintDsAst).map(ii => indentStr(1, ii)).join('\n')}`;
        case 'predicate_call':
...
function pprintExprListAst(ast: ExpressionDsAst[], withtype: 'withtype' | 'without_type' = 'without_type',
    withio: 'withio' | 'without_io' = 'without_io'): string {
    return ast.map(ee => pprintExprAst(ee, withtype, withio)).join(', ');
}
function pprintExprAst(ast: ExpressionDsAst, withtype: 'withtype' | 'without_type' = 'without_type',
    withio: 'withio' | 'without_io' = 'without_io'): string {
    switch (ast.type) {
        case 'identifier':
