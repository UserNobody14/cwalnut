...
function listValsToList(
...
): Effect.Effect<Expr, never, ExpressionDesugarServices> {
	const src =
		listVals.length > 0
			? tocloc(listVals[0])
			: defaultCodeLocation;
	...
	return Effect.gen(function* () {
		const hasSplats = listVals.some(
			(n) => n.grammarType === "splat",
		);
		if (!hasSplats) {
...
export function parseExpr(
	...
	switch (node.type) {
		...
		case "binary_operator":
			return Effect.gen(function* () {
				const left = yield* parseExpr(
					node.children[0],
					unifyVar,
				);
				const op = node.children[1].text;
				const right = yield* parseExpr(
					node.children[2],
					unifyVar,
				);
				const val = unifyVar ?? (yield* genFresh());
		...
		case "dictionary":
			return Effect.gen(function* () {
				...
				type Kv = [Expr, Expr, CodeLocation];
				const lvd: Kv[] = [];
		...
		default:
			return Effect.die(
				new Error(
					`parseExpr: unsupported node type ${node.type}`,
				),
			);
	}
}
