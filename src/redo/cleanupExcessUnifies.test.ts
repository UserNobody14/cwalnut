import {
	...
	filterOutUnifyCalls,
	filterUnifyPredicateCalls,
	groupUnifies,
	refactorTermsToMergeUnifies,
	refactorUnifications,
...
describe.skip("cleanupExcessUnifies", () => {
	test("returns terms unchanged with pass flag true", () => {
		const terms: TermGeneric<"info"> = make.predicate_call(
...
describe.skip("refactorTermsToMergeUnifies 1", () => {
	const { a, b, c, d, e, g, h, i, z, qq, dd } = make.lvar2(
		"info" as const,
	);
	...
	const { empty, rest, first, appendo_recur_0 } =
	...
	test("preserves non-unify terms", () => {
		...
		expect(result).toEqual([
			make.conjunction([unify(a, b), other()]),
		]);
	});
	...
	test("With pred def", () => {
		const terms: TermGeneric<"info"> = make.conjunction([
			...
			make.predicate_definition(
				d,
				[z, e],
				conjunction1(unify(z, e), unify(qq, dd)),
			),
			other(),
		]);
		const [result, _] = refactorTermsToMergeUnifies(terms);
	...
	test("With disjunction", () => {
		const terms: TermGeneric<"info"> = make.conjunction([
			...
			make.disjunction([
				other(),
				conjunction1(unify(g, h), unify(g, i)),
			]),
		]);
		const [result, _] = refactorTermsToMergeUnifies(terms);
		// Expect a single unify term with merged variables
	...
	test("With more advanced predicate", () => {
		...
		expect(pprintQuick(result)).toEqual(
			pprintQuick(exTerm),
		);
		// Expect a single unify term with merged variables
		expect(result).toEqual(exTerm);
...
describe("refactor unification array", () => {
	...
	test("preserves non-unify terms", () => {
		...
		// Expect both the merged unify term and the non-unify term to be present
		expect(pprintQuick(result)).toEqual(
			pprintQuick(make.conjunction([unify(a, b), other()])),
		);
	});
});
...
describe.skip("simplifyUnifyChain", () => {
	...
	test("cleans up simple unneeded unify chain", () => {
		...
		expect(result).toEqual([unify(a, c)]);
	});
});
...
describe.skip("simplify example unification", () => {
	const {
		a,
		b,
		c,
	...
	const { unify, other, empty } = make.pred2(
		"info" as const,
	);
	const term = conjunction1(
		other(ab_2_disj_0),
	...
	test("groupUnifies", () => {
		const utem = filterUnifyPredicateCalls(term.terms);
		expect(utem).toEqual([
			unify(a_0_disj_0Z_51, a_0_disj_0Y_51, a_0_disj_0),
			unify(b_1_disj_0Z_53, b_1_disj_0Y_53, b_1_disj_0),
			unify(ab_2_disj_0Z_55, ab_2_disj_0Y_55, ab_2_disj_0),
		...
		const rs = groupUnifies(utem);
		if (!rs)
			throw new Error(
				"Expected groupUnifies to return a value",
			);
		expect(pprintQuick(rs)).toEqual(
			pprintQuick([
				// unify(a_0_disj_0Z_51, a_0_disj_0Y_51, a_0_disj_0),
