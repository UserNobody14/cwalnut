...
type TermModeDefinitionMap = ImmMap<
	string,
	ImmMap<string, PredicateDefinitionGeneric<ModeDetType>>
>;
...
export function* termToModeWorlds(
	...
	switch (term.type) {
		case "conjunction": {
			const processedTermGens = term.terms.map((t) =>
				termToModeWorlds(t, pm),
			);
			const processedTerms = mergePossibilities(
				processedTermGens,
			);
			const eTermList = mapStreams((sp) => {
				return reduceStreamArray(sp, (a, b) =>
					mergeModeWorlds(a, b),
				);
			}, processedTerms);
			yield* mapStreams((wrld) => {
				// Wrap term arrangement in conjunction
				return [
					{
						...wrld,
						termArrangement: [
							conjunction1(...wrld.termArrangement),
						],
					},
				];
			}, eTermList);
		...
		case "disjunction": {
			...
			const et = term.terms.map((t) =>
				termToModeWorlds(t, pm),
			);
			const tSet = mergePossibilities(et);
			for (const t of tSet) {
				yield* wrapMWorlds(t, (tr) => [
					disjunction1(...tr),
				]);
			}
			break;
		...
		case "fresh": {
			const processedWorlds = termToModeWorlds(
				term.body,
				pm,
			);
			const compressibles = mapStreams(
				(world): Iterable<ModeWorld> => {
					const newVarModes = term.newVars.reduce(
						(acc, v) => {
							return acc.set(v.value, {
								from: "free",
								to: "free",
							});
						},
					...
					const nvp = mergeVarModes(
						newVarModes,
						world.varModes,
					);
					if (!nvp) {
						console.warn(
							`Invalid var modes for fresh: ${term.newVars.map((v) => v.value).join(", ")}`,
						);
						return [];
					}
					return [
						{
							...
							termArrangement: [
								{
									...term,
									newVars: term.newVars.map<
										IdentifierGeneric<ModeDetType>
									>((v) => ({ ...v, info: nullModeType })),
									body: conjunction1(
										...world.termArrangement,
									),
								},
							],
							det: world.det,
			...
			return compressAcrossVars(
				compressibles,
				term.newVars.map((nv) => nv.value),
			);
		}
		case "predicate_call": {
			...
			if (!predModes) {
				console.warn(
					`No predicate modes for ${term.source.value}`,
				);
				return [];
			}
			const mappedCalls = predModes.map((md) =>
				predCallToModeWorld(md, term, pm),
			);
		...
		case "predicate_definition": {
			...
			const worldsForBody = compressAcrossVars(
				termToModeWorlds(term.body, pm),
				term.args.map((arg) => arg.value),
			);
			const predName = term.name.value;
			const mdwList: ModeDetType[] = [];
			const defs = ImmMap<
				string,
				PredicateDefinitionGeneric<ModeDetType>
			>();
			for (const world of worldsForBody) {
				// Extract all of the args with their var-modes in order
				const args = listVarModes(
					world.varModes,
					term.args,
				);
				...
				defs.set(
					modeDetTypeToString({ varModes: args, det }),
					{
						...term,
						name: {
							...term.name,
							info: { varModes: args, det },
						},
						args: term.args.map((arg, i) => ({
							...arg,
							info: nullModeType,
						})),
...
const wrapMWorlds = (
	world: ModeWorld[],
	tfn: (
		tr: TermGeneric<ModeDetType>[],
	) => TermGeneric<ModeDetType>[],
): ModeWorld[] => {
	const endTrs = tfn(
		world.flatMap((w) => w.termArrangement),
	);
	return world.map((w) => {
		return {
			...w,
...
function* predCallToModeWorld(
	mode: ModeDetType,
	term: PredicateCallGeneric<Type>,
	pm: ImmMap<string, ModeDetType[]>,
): Iterable<ModeWorld> {
	...
	const varModes = term.args.reduce<ImmMap<
		string,
		Mode
	> | null>((acc, arg, i) => {
		if (!acc) return null;
...
function* modeWorldConjoin(
	worlds: Iterable<ModeWorld[]>,
): Iterable<ModeWorld> {}

function* reduceModeWorlds(
	worlds: ModeWorld[],
): Iterable<ModeWorld> {}
...
export function mergeModeWorldList(
	...
	return mergeModeWorldList([
		mergeModeWorldList(left),
		mergeModeWorldList(right),
	]);
}
...
export function* mergeModeWorlds(
	...
	if (varModes) {
		yield {
			varModes,
			polymorphModes: mergePolymorphModes(
				world1.polymorphModes,
				world2.polymorphModes,
			),
			termArrangement: world1.termArrangement.concat(
				world2.termArrangement,
			),
	...
	if (varModes2) {
		yield {
			...
			termArrangement: world2.termArrangement.concat(
				world1.termArrangement,
			),
...
function mergeModeDetTypeLists(
	modes1: ModeDetType[],
	modes2: ModeDetType[],
) {
...
function mergePolymorphModes(
	world1: ImmMap<string, ModeDetType[]>,
	world2: ImmMap<string, ModeDetType[]>,
) {
	let nv: ImmMap<string, ModeDetType[]> =
		world1.merge(world2);
	for (const [key, modes] of world1.entries()) {
		if (world2.has(key)) {
			...
			nv = nv.set(
				key,
				mergeModeDetTypeLists(modes, modes2),
			);
		}
	}
	return nv;
...
function mergeDefinitions(
	defs1: TermModeDefinitionMap,
	defs2: TermModeDefinitionMap,
) {
