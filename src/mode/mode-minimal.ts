...
const to_typed_free = (v: string): Wrapped =>
	to_wrap_state("free", to_simple_state(v));
const to_typed_grounded = (v: string): Wrapped =>
	to_wrap_state("ground", to_simple_state(v));
const initialize_fresh = to_simple_state("initialized");

const to_initialization_mode = (
	v: string,
): ModeMinimal => ({
...
function resolveVariableName<T>(
	varName: string | IdentifierGeneric<T>,
) {
	let varVal: string;
	if (typeof varName === "string") {
		varVal = varName;
...
function unifyStVar(
	...
	if (
		st1.type === "type_variable" &&
		st2.type === "type_variable"
	) {
		...
		if (substitutions.has(st1.lvar)) {
			return unifyStVar(
				substitutions.get(st1.lvar, st2),
				st2,
				substitutions,
			);
		}
		if (substitutions.has(st2.lvar)) {
			return unifyStVar(
				st1,
				substitutions.get(st2.lvar, st1),
				substitutions,
			);
		}
...
function unifyModes(
	...
	const newModeMap: VarModeMapMinimal = [
		substitutions[0],
		newSubs,
	];
	const newMode = to_mode(
		walkVarState(newModeMap, mode1.from),
		walkVarState(newModeMap, newState),
	);
...
function mergeModes(
	...
	const newModeMap2: VarModeMapMinimal = [
		newModeMap[0],
		newSubs2,
	];
	return [
...
function applyVarState(
	...
	if (newPredSource.type === "polymorphic_pred") {
		...
		return [
			to_polymorphic_pred(noutv[0], newDet),
			noutv[1],
		];
...
const mergeVarModeMaps = (
	...
	for (const [key, value] of substitutions2[1]) {
		if (sOut[1].has(key)) {
			const ustv = unifyStVar(
				sOut[1].get(key, value),
				value,
				sOut[1],
			);
			if (ustv === null) {
...
function convertTerms<T>(
	...
	currentModeMap: VarModeMapMinimal,
) => Iterable<[TermGeneric<T>[], VarModeMapMinimal]> {
	type TermsOut = [TermGeneric<T>[], VarModeMapMinimal];
			...
			switch (term.type) {
				...
				case "disjunction": {
					const termList = mergePossibilities(
						term.terms.map((t) =>
							convertTerms([t])(currentModeMap),
						),
					);
					for (const nzzz of termList) {
						const mappedOut = nzzz.flatMap((tr) =>
							conjunction1(...tr[0]),
						);
						const mappedOut2 = nzzz.map((t) => t[1]);
						yield [
							[disjunction1(...mappedOut)],
							mappedOut2.reduce(
					...
					const newModeMap = term.newVars.reduce(
						(acc, v) =>
							setMode(
								acc,
								v.value,
								to_initialization_mode(v.value),
							),
						currentModeMap,
					);
					yield* mapStreams<TermsOut, TermsOut>(
				...
				case "predicate_call": {
					const predSource = getStvar(
						currentModeMap,
						term.source.value,
					);
					const argModes = term.args.map((arg) =>
						getVarMode(currentModeMap, arg),
					);
					const noutv = applyVarState(
						predSource,
						argModes,
						currentModeMap,
					);
